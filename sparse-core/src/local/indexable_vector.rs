...
pub struct LocalIndexableVector<'a, T: Scalar> {
    data: Option<RwLock<LocalIndexableVectorData<T>>>,
    index_layout: &'a LocalIndexLayout,
}

pub struct LocalIndexableVectorData<T: Scalar> {
    data: Vec<T>,
}
...
impl<'a, T: Scalar> LocalIndexableVector<'a, T> {
    pub fn new(index_layout: &'a LocalIndexLayout) -> LocalIndexableVector<'a, T> {
        LocalIndexableVector {
            data: Some(RwLock::new(LocalIndexableVectorData {
...
impl<T: Scalar> MultSumInto for LocalIndexableVector<'_, T> {
    ...
    fn mult_sum_into(&mut self, other: &Self, scalar: Self::T) -> sparse_traits::types::Result<()> {
        ...
        if scalar == T::one() {
            for (first, second) in my_data.iter_mut().zip(other_view.iter()) {
                *first += *second;
            }
            return Ok(());
        }
...
mod tests {
    ...
    fn test_inner() {
        ...
        let vec1 = new_vec::<c64>(&index_layout);
...

        {
            ...
            *vec2_data.get_mut(0).unwrap() = c64::new(2.0, 3.0);
            *vec2_data.get_mut(1).unwrap() = c64::new(0.4, 1.5);
        }
    ...
    fn abs_square_sum() {
        ...
        *vec_data.get_mut(0).unwrap() = val1;
        *vec_data.get_mut(1).unwrap() = val2;
